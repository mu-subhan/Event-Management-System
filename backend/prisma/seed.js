...
async function main() {
      ...
      .map((_, index) => {
        ...
        return prisma.user.create({
          data: {
            ...volunteerData[index],
            password: hashedPassword,
            contactNumber: `+92-300-${Math.floor(
              1000000 + Math.random() * 9000000
            )}`,
            role: "Volunteer",
            experienceYears: Math.floor(1 + Math.random() * 5),
            description: `Dedicated volunteer with experience in ${volunteerData[
              index
            ].skills.join(", ")}`,
          },
        });
  ...
  console.log(
    `✅ Created ${volunteers.length + 1} users (1 admin, ${
      volunteers.length
    } volunteers)`
  );
  ...
  const events = await Promise.all(
    eventTypes.map((type, index) => {
      const startTime = new Date(
        currentDate.getTime() + index * 2 * 24 * 60 * 60 * 1000
      );
            ...
            create: [
              {
                ...
                maxVolunteers: 2,
                volunteers: {
                  connect: [{ id: volunteers[index % volunteers.length].id }],
                },
              },
              {
    ...
    ...volunteers.map((volunteer, index) =>
      prisma.notification.create({
        data: {
          ...
          message: `You have been assigned to the ${
